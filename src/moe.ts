...
export interface MoeRoute {
  app: string;          // fabric app name, e.g. "@git-fabric/proxmox"
  toolPrefix: string;   // tool name prefix, e.g. "pve_"
  keywords: string[];
  priority: number;     // base score weight per keyword match
}
...
export interface MoeResult {
  ...
  matches: MoeMatch[];  // sorted by score descending
  top: MoeMatch | null;
}
 ...
 */
export function routeQuery(query: string): MoeResult {
  const lower = query.toLowerCase();
  const matches: MoeMatch[] = [];
  ...
  let action: MoeResult["action"] = "none";
  if (top) {
    action = top.confidence >= THRESHOLD_ROUTE ? "route" : "suggest";
    if (top.confidence < THRESHOLD_SUGGEST) action = "none";
  }
